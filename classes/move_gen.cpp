...
U8 MoveGen::GetAllMoves(std::array<Move, MAXMOVECOUNT> *moves, Board *board, U64 attackedSquares) {
    ...
    moveCount += GetKingMoves(moves, moveCount, board, attackedSquares);

    return moveCount;
}

U8 MoveGen::GetPawnMoves(std::array<Move, MAXMOVECOUNT> *moves, int startIndex, Board *board) {
    ...
    while (pieces) {
        ...
        while (captures) {
            ...
            if (IsKingSafe(board, (board->GetOccupiedBB() ^ C64(lsb)) | C64(capturePiece), board->GetColorBB(oppColor) ^ C64(capturePiece))) {
                // If Promotion
        ...
        if (!(board->GetOccupiedBB() & pawnSingleMove[(int) lsb]))
            if (IsKingSafe(board, (board->GetOccupiedBB() ^ C64(lsb)) | BitShifts::Shift(C64(lsb), up, 1))) {
                // If Promotion
                if (C64(lsb) & (U64) promotionRank) {
        ...
        if (C64(lsb) & (U64) doubleRank && !(board->GetOccupiedBB() & pawnDoubleMove[(int) lsb]))
            if (IsKingSafe(board, (board->GetOccupiedBB() ^ C64(lsb)) | BitShifts::Shift(C64(lsb), up, 2)))
...
U8 MoveGen::GetRookMoves(std::array<Move, MAXMOVECOUNT> *moves, int startIndex, Board *board) {
    int moveCount = 0;

    U64 pieces = board->GetPiecePos(color, PieceType::Rook);
    for (const auto & rookDirection : rookDirections)
        moveCount += GetMoves(moves, startIndex + moveCount, board, pieces, rookDirection);
    return moveCount;
}

U8 MoveGen::GetBishopMoves(std::array<Move, MAXMOVECOUNT> *moves, int startIndex, Board *board) {
    int moveCount = 0;

    U64 pieces = board->GetPiecePos(color, PieceType::Bishop);
    for (const auto & bishopDirection : bishopDirections)
        moveCount += GetMoves(moves, startIndex + moveCount, board, pieces, bishopDirection);
    return moveCount;
}

U8 MoveGen::GetQueenMoves(std::array<Move, MAXMOVECOUNT> *moves, int startIndex, Board *board) {
    int moveCount = 0;

    U64 pieces = board->GetPiecePos(color, PieceType::Queen);
    for (const auto & queenDirection : queenDirections)
        moveCount += GetMoves(moves, startIndex + moveCount, board, pieces, queenDirection);
    return moveCount;
}

U8 MoveGen::GetKnightMoves(std::array<Move, MAXMOVECOUNT> *moves, int startIndex, Board *board) {
    ...
    while (pieces) {
            ...
            while (attackMoves) {
                ...
                if (IsKingSafe(board, (board->GetOccupiedBB() ^ C64(lsbPiece)) | C64(lsb), board->GetColorBB(oppColor) ^ C64(lsb)))
            ...
            while (quietMoves) {
                ...
                if (IsKingSafe(board, (board->GetOccupiedBB() ^ C64(lsbPiece) | C64(lsb))))
...
U8 MoveGen::GetKingMoves(std::array<Move, MAXMOVECOUNT> *moves, int startIndex, Board *board, U64 attackedSquares) {
    ...
    while (attackMoves) {
        U64 lsb = Utilities::LSB_Pop(&attackMoves);
        if (IsKingSafe(board, board->GetOccupiedBB() ^ C64(lsbPiece), board->GetColorBB(oppColor) ^ C64(lsb), C64(lsb)))
            AppendMove(moves, startIndex + moveCount, &moveCount, Move(MoveType::Capture, (Square) lsbPiece, (Square) lsb, board->GetType((Square) lsb)));
    }
...
U8 MoveGen::GetMoves(std::array<Move, MAXMOVECOUNT> *moves, int startIndex, Board *board, U64 pieces,
                     Direction direction) {
    U8 moveCount = 0;
    int counter = 1;
    ...
    while (pieces) {
        ...
        while (attackMoves) {
            U64 lsb = Utilities::LSB_Pop(&attackMoves);
            if (IsKingSafe(board, (board->GetOccupiedBB() ^ BitShifts::Shift(C64(lsb), Utilities::GetOppositeDirection(direction), counter)) | C64(lsb), board->GetColorBB(oppColor) ^ C64(lsb)))
        ...
        while (quietMoves) {
            ...
            if (IsKingSafe(board, (board->GetOccupiedBB() ^ BitShifts::Shift(C64(lsb), Utilities::GetOppositeDirection(direction), counter) | C64(lsb))))
