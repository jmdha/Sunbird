...
NEXT_TOKEN:
    bool inApro = (s[0] == '"');
    if (auto p = s.find_first_of((inApro) ? '"' : ' ', (inApro) ? 1 : 0);
        p != std::string::npos) {
    ...
    while (true) {
        ...
        case Command::position:
            if (tokens[1] == "startpos") {
                ...
                std::string fen =
                    input.substr(fenIndex, (movesIndex == std::string::npos)
                                               ? input.size() - fenIndex
                                               : movesIndex - fenIndex - 1);
                board = Import::FEN(fen);
            } else
                throw std::logic_error("Unexpected token in position: \"" +
                                       tokens[1] + '\"');

            if (auto p = input.find("moves"); p != std::string::npos)
                Import::MoveSequence(board,
                                     input.substr(p + 6, input.size() - p - 6));
